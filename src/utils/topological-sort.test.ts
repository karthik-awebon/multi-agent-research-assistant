...
describe('computeTopologicalState', () => {
  it('identifies unblocked nodes with no dependencies', () => {
    const state: ExecutionGraphState = {
      nodes: {
        '1': { id: '1', type: 'TASK', status: 'PENDING', name: 'Task 1' },
      },
      edges: {},
    };
    ...
    expect(unblockedNodes[0].id).toBe('1');
  ...
  it('blocks nodes whose dependencies are not completed', () => {
    const state: ExecutionGraphState = {
      nodes: {
        '1': { id: '1', type: 'TASK', status: 'RUNNING', name: 'Task 1' },
        '2': { id: '2', type: 'TASK', status: 'PENDING', name: 'Task 2' },
      },
      edges: {
        '1->2': { id: '1->2', source: '1', target: '2' },
      },
    };

    const { unblockedNodes, blockedNodes } = computeTopologicalState(state);
    expect(unblockedNodes.map(n => n.id)).toContain('1');
    expect(blockedNodes.map(n => n.id)).toContain('2');
  });

  it('unblocks nodes when all dependencies are completed', () => {
    const state: ExecutionGraphState = {
      nodes: {
        '1': { id: '1', type: 'TASK', status: 'COMPLETED', name: 'Task 1' },
        '2': { id: '2', type: 'TASK', status: 'PENDING', name: 'Task 2' },
      },
      edges: {
        '1->2': { id: '1->2', source: '1', target: '2' },
    ...
    const { unblockedNodes, blockedNodes } = computeTopologicalState(state);
    expect(unblockedNodes.map(n => n.id)).toContain('1');
    expect(unblockedNodes.map(n => n.id)).toContain('2');
    expect(blockedNodes).toHaveLength(0);
  });
  ...
  it('handles complex branching dependencies', () => {
    ...
    // After 3 completes
    state.nodes['3'].status = 'COMPLETED';
    const result2 = computeTopologicalState(state);
